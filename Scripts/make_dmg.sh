#!/usr/bin/env bash
# Builds a polished .pkg-based DMG for Maccopy.
#
# Install flow for users:
#   1. Open DMG  →  double-click "Install Maccopy.pkg"
...
VOLUME_NAME="Maccopy $VERSION"
DMG_PATH="$OUTPUT_DIR/Maccopy-${VERSION}.dmg"
...
PKG_COMPONENT="$BUILD_DIR/Maccopy-component.pkg"
PKG_FINAL="$BUILD_DIR/Maccopy-${VERSION}.pkg"
...
/usr/bin/defaults delete com.maccopy.maccopy hasCompletedSetup 2>/dev/null || true
...
<installer-gui-script minSpecVersion="2">
    ...
    <pkg-ref id="$BUNDLE_ID" version="$VERSION" onConclusion="none">Maccopy-component.pkg</pkg-ref>
...
\f0\fs26\b Maccopy $VERSION\b0\
\
\fs22 Maccopy — clipboard history for macOS. Text, images and files in your menu bar.\
\
\b What this installer does:\b0\
\
\pard\tx220\li220\fi-220
\f0 \'95 Copies Maccopy.app to /Applications\
...
cp "$PKG_FINAL" "$STAGING/Install Maccopy.pkg"
...
tell application "Finder"
    tell disk "$VOLUME_NAME"
        ...
        set position of item "Install Maccopy.pkg" of container window to {270, 195}
...
echo "  Install: open DMG → double-click 'Install Maccopy.pkg'"
echo
